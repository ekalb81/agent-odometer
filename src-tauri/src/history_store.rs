...
mod tests {
    ...
    /// Issue #182's discriminating experiment: does the bulk-scan memory
    /// peak scale with `SCAN_WRITE_BATCH_SIZE`?
    ///
    /// The issue names two candidate explanations for a ~1,491 MB peak that
    /// imply opposite fixes:
    ///
    /// - **Queue model** — workers stalled on the durable-write mutex hold
    ///   their batches alive, so the peak tracks batch size times waiters.
    /// - **In-flight-window model** — the peak is simply bytes being
    ///   processed concurrently, so it flattens once the window saturates.
    ///
    /// This sweeps 16 / 64 / 256 with concurrent writers (mirroring
    /// `scan_all`'s `par_chunks`), at realistic per-session size, reporting
    /// peak heap and the write-lock contention counters #195 added.
    ///
    /// Fidelity limit, stated because it bounds the conclusion: this is a
    /// synthetic corpus in a temp directory, not the field's 4,687-file
    /// scan, and it exercises only the write path — no parse, no cache
    /// lookup. It can show whether *this* path's peak responds to batch
    /// size; it cannot reproduce the field's absolute numbers.
    #[test]
    #[ignore = "issue #182 batch-size sweep; run with `cargo test --release --lib                 probe_scan_write_batch_size_sweep -- --ignored --nocapture` (filtering to                 this exact test name keeps the process-wide counting allocator from also                 seeing another concurrently-running test's allocations)"]
    fn probe_scan_write_batch_size_sweep() {
        use rayon::prelude::*;

        const SESSIONS: usize = 256;
        const HOT_SESSIONS: usize = 26;
        ...
        for batch_size in [16usize, 64, 256] {
            ...
            let paths: Vec<PathBuf> = (0..SESSIONS)
                .map(|index| PathBuf::from(format!("sweep-{index}.jsonl")))
                .collect();
            let items: Vec<(&Path, &Session, i64)> = paths
                .iter()
                .zip(sessions.iter())
            ...
            let elapsed = started.elapsed();
            let peak = crate::memory::heap_peak_bytes_raw();
            crate::memory::configure_heap_tracking(false);

            eprintln!(
                "batch={batch_size:>3}  peak_heap={:>9.1} MB  wall={:>7.0} ms  peak/batch={:>7.2} MB",
                peak as f64 / (1024.0 * 1024.0),
                elapsed.as_secs_f64() * 1_000.0,
                peak as f64 / (1024.0 * 1024.0) / batch_size as f64,
            );
        }
    }
