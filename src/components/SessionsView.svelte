            ...
            <tbody>
              {#each modelComparison as metric (`${metric.harness}:${metric.model}`)}
                <tr class="border-t border-edgerow">
                  <td class="py-1.5 text-ink"><span class="text-ink-faint">{providersStore.displayName(metric.harness)}</span> · {metric.model}{#if metric.unpriced}<span class="text-amber-500" title="Excluded because no published rate is available"> ◇</span>{:else if metric.basis === 'aliased'}<span class="text-sky-500" title="Priced via a model-id alias, not a direct rate-card match"> ↝</span>{:else if metric.basis === 'floating_alias'}<span class="text-sky-500" title="Priced via a floating provider alias — correct as of the rate card's fetch, but the provider repoints these without renaming, so it expires and falls back to a flagged fallback rate"> ↝*</span>{:else if metric.basis === 'estimated'}<span class="text-sky-500" title="Includes cache-write tokens priced at the ordinary input rate — no published cache-write premium for this model"> ≈</span>{:else if metric.fallbackUsed}<span class="text-amber-500" title="Configured fallback rate used"> ⚠</span>{/if}</td>
